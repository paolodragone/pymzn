...
def _py_output_stmt(output_vars):
    stmt = ['output [ "{", ']
    for i, var in enumerate(output_vars):
        if i > 0:
            stmt.append(', ')
        stmt.append("'{0}': show({0})".format(var))
    stmt.append('}];')
    return ''.join(stmt)
...
class MiniZincModel(object):
    ...
    def __init__(self, file_mzn=None, output_vars=None):
    ...
    def compile(self):
        ...
        if self.file_mzn:
            ...
            if isinstance(mzn, str):
                if mzn.endswith('.mzn') and os.path.exists(mzn):
                    with open(self.file_mzn) as f:
                        model = f.read()
                else:
                    model = mzn
            else:
                self.log.warning('The provided value for file_mzn is not a '
        ...
        for i, (var, (vartype, val, comment)) in enumerate(self.vars.items()):
            if i == 0:
                lines.append('\n')
            if comment:
        ...
        for i, (constr, comment) in enumerate(self.constraints):
            if i == 0:
                lines.append('\n')
            if comment:
                lines.append('% {}'.format(comment))
        ...
        lines.append('\n')
        output_stmt = _py_output_stmt(self.output_vars)
